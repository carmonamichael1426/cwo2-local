...
class ProformaVSPI_model extends CI_Model {
    ...
    public function uploadLine($itemCode,$desc,$uom,$qty,$directUnitCost,$unitCostLcy,$amt,$amtIncVat,$unitCost,$lineAmt,$qtyPerUom,$uomCode,$piId,$lineDiscPer,$lineDiscAmt,$free)
    {
    ...
    {
        $query = $this->db->select('head.pi_head_id as piId, head.vendor_invoice_no, head.pi_no as pi_no, head.posting_date as date , head.status as status, head.audited, head.supplier_id, c.customer_name as customer_name, p.po_no as po_no, p.posting_date AS piDate, cm.cm_head_id, cm.cm_no, cm.posting_date, cm.amt_including_vat,l.crf_line_id,l.pi_head_id')
    ...
    {
        $query = $this->db->select('h.cm_head_id,h.cm_no,h.posting_date,pi.pi_no,po.po_no,l.item_code,l.description,l.uom,l.qty,l.direct_unit_cost,i.item_department_code,i.itemcode_loc,i.item_group_code')
                          ->from('cm_line l')
                          ->join('cm_head h', 'h.cm_head_id = l.cm_head_id', 'inner')
                          ->join('purchase_invoice_header pi', 'pi.pi_head_id = h.pi_head_id', 'inner')
                          ->join('po_header po', 'po.po_header_id = pi.po_header_id', 'inner')
    ...
    public function getVarianceAdjustments($crfId)
    {
        $query = $this->db->select('v.variance_id, a.*')
                          ->from('variance v')
                          ->join('variance_adjustment a', 'a.variance_id = v.variance_id', 'inner')
                          ->where('v.crf_id', $crfId)
                          ->order_by('v.variance_id')
                          ->get();
        return $query->result_array();
    }
    ...
    public function getPurchaseInvoiceItems($piId, $supId)
    {
        $query  = $this->db->query('SELECT h.pi_head_id AS piId, h.posting_date AS piDate, h.vendor_invoice_no, h.pi_no AS piNo, po.po_header_id AS poId, po.po_no,
                                   ...
                                   INNER JOIN purchase_invoice_line l ON l.pi_head_id = h.pi_head_id 
                                   LEFT JOIN items i ON i.itemcode_loc = l.item_code 
                                   WHERE h.pi_head_id = '.$piId.' AND i.supplier_id = '.$supId.' AND (l.free = 0 OR l.free IS NULL) GROUP BY l.item_code');
        return $query->result_array();
    }

    public function getPiFreeItems($piId, $supId)
    {
        $query  = $this->db->query('SELECT h.pi_head_id AS piId, h.posting_date AS piDate, h.vendor_invoice_no, h.pi_no AS piNo, po.po_header_id AS poId, po.po_no,
                                   i.id,l.item_code AS item, l.description AS idesc, l.uom, sum(l.qty) AS qty, 
                                   l.direct_unit_cost AS direct, l.amt_including_vat AS amount,i.itemcode_loc,i.item_division, i.item_department_code, i.item_group_code
                                   ...
                                   INNER JOIN purchase_invoice_line l ON l.pi_head_id = h.pi_head_id 
                                   LEFT JOIN items i ON i.itemcode_loc = l.item_code 
                                   WHERE h.pi_head_id = '.$piId.' AND i.supplier_id = '.$supId.' AND l.free = 1 GROUP BY l.item_code');
        return $query->result_array();
    }

    public function searchPo($supId,$string)
    {
        $query = $this->db->select('*')
                          ->from('po_header')
    ...
    public function getPiStatistics()
    {
