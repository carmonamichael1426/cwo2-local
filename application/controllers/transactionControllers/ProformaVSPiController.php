...
class ProformaVSPiController extends CI_Controller
{
                ...
                for ($l = 0; $l < $totalLine; $l++) 
                {
                    if ($line[$l] != NULL) {
                        ...
                        if ($countRefined == 20 && $refined[6] != 0 ) { // line and qty != 0
                            if( trim($refined[6]) != 0 && trim($refined[7]) == 0){
                                $free = 1;
                            } else if( trim($refined[6]) != 0 && trim($refined[7]) != 0 ){
                                $free = 0;
                            }
                            $piNo               = trim($refined[0]);
                            ...
                            $this->proformavspi_model->uploadLine($itemCode, $desc, $uom, $qty, $directUnitCost, $unitCostLcy, $amt, $amtIncVat, $unitCost, $lineAmt, $qtyPerUom, $uomCode, $getPiId,$lineDiscPercent,$lineDiscAmount,$free);
                        }
                    }
                }
            }
    ...
    public function untagPiFromCrf()
    {
        ...
        if($query->num_rows() > 0)
        {
            ...
            if($untagFromCrfLine)
            {
                ...
                $getPiNo = $this->proformavspi_model->getPiHead($fetch_data['piId'])['pi_no'];
    ...
    public function matching2()
    {
        ...
        $freeProfItems        = array();
        $freePiItems          = array();
        $flatFreeProfItems    = array();
        $flatFreePiItems      = array();
        $flatCmLineDetails    = array();
        ...
        foreach( $getProf as $get1 ){
            ...
            $freeProfItems[]    = $this->proformavspi_model->freeItems($get1['proforma_header_id'], $fetch_data['supId']);
        ...
        foreach( $getPi as $get2 ){
            ...
            $freePiItems[]            = $this->proformavspi_model->getPiFreeItems($get2['pi_head_id'], $fetch_data['supId']);
        ...
        if( !empty($freeProfItems)){
            $flatFreeProfItems   = call_user_func_array('array_merge', $freeProfItems);
        }

        if( !empty($freePiItems)){
            $flatFreePiItems   = call_user_func_array('array_merge', $freePiItems);
        }

        // dump($flatFreeProfItems);
        // dump($flatFreePiItems);
        // die();
                    ...
                    foreach($getDeals as $deals){
                        ...
                        if($deals['number'] == $itemCodeToFind ){
                            if($getDiscType == "PERCENTAGE"){
                                ...
                                if( $getNoOfDisc == "2"){
                                    $disc2 = 1.00 - $deals['disc_2'] * 0.01 ;                               
                                } else if(  $getNoOfDisc == "3" || $fetch_data['supId'] == '9' ){
                                    $disc2 = 1.00 - $deals['disc_2'] * 0.01 ;
                                    $disc3 = 1.00 - $deals['disc_3'] * 0.01 ;
                                } else if(  $getNoOfDisc == "4" ){
                                    $disc2 = 1.00 - $deals['disc_2'] * 0.01 ;
        ...
        if(!empty($flatCmLineDetails)){
            ...
            {      
                ...
                $cmLineMerge[] = ['cmId' => $cml['cm_head_id'], 'cmNo' => $cml['cm_no'], 'date' => $cml['posting_date'], 'ref' => $cml['pi_no'], 'po' => $cml['po_no'],
                                  'item' => $cml['item_code'],'desc' => $cml['description'],'uom' => $cml['uom'],'qty' => $cml['qty'], 'price' => $cml['direct_unit_cost'],
                                  'disc1'=> $disc1, 'disc2' => $disc2, 'disc3' => $disc3, 'disc4' => $disc4, 'disc5' => $disc5, 'disc6' => $disc6, 'piNo' => $cml['pi_no']  ];
        ...
        $transactionId = $this->db->insert_id();
        if($type == "pdf"){
            $file = $this->printPDFMatching2($transactionId,$profLineMerge, $purchLineMerge, $cmLineMerge, $flatFreeProfItems, $flatFreePiItems, $fetch_data);
    ...
    public function printPDFMatching2($transactionId,$profLineMerge, $purchLineMerge, $cmLineMerge, $flatFreeProfItems, $flatFreePiItems, $fetch_data)
    {
        $supplier             = $this->proformavspi_model->getSupplierData($fetch_data['supId'],'supplier_id')['supplier_name'];
        $getPricing           = $this->proformavspi_model->getSupplierData($fetch_data['supId'],'supplier_id')['pricing'];
        $getAmounting         = $this->proformavspi_model->getSupplierData($fetch_data['supId'],'supplier_id')['amounting'];
        ...
        $sopCharges           = $this->proformavspi_model->getSopCharges($fetch_data['crfId']); 
        $mentionsFromSop      = $this->proformavspi_model->getVarianceMentions($fetch_data['crfId']);
        $getVarianceAdj       = $this->proformavspi_model->getVarianceAdjustments($fetch_data['crfId']);
        ...
        $sopVarianceMentionTotal = 0;
        $varianceDiscountedTotal = 0;
        $varianceAdjustmentTotal = 0;
        ...
        $this->createProfTableHeader($pdf, 'Pro-forma Sales Invoice');
        ...
        if(!empty($flatFreeProfItems)){
            $this->createProfTableHeader($pdf, 'Pro-forma Sales Invoice - Free Item(s)');
            foreach( $flatFreeProfItems as $freeProf){

                $pdf->cell(80, 6, $freeProf['profCode'], 'LBR', 0, 'C');
                $pdf->cell(35, 6, $freeProf['po_no'], 'BR', 0, 'C', 0);
                $pdf->cell(20, 6, $freeProf['item'], 'BR', 0, 'C', 0);
                $pdf->cell(120,6, $freeProf['idesc'], 'BR', 0, 'L', 0);
                $pdf->cell(15, 6, $freeProf['uom'], 'BR', 0, 'C', 0);
                $pdf->cell(12, 6, $freeProf['qty'], 'BR', 0, 'C', 0);
                $pdf->cell(27, 6, 0, 'BR', 0, 'R', 0); //net price
                $pdf->cell(30, 6, 0, 'BR', 0, 'R', 0); //discounted price
                $pdf->cell(30, 6, 0, 'BR', 0, 'R', 0);  //gross price
                $pdf->cell(32, 6, 0, 'BR', 0, 'R', 0);  //net amount
                ...
                $freeProfItemQty += $freeProf['qty'] ;
                $freeProfItemCount ++;
                $pdf->Ln();
            }
            $pdf->setFont('Arial', 'B', 10);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->cell(270, 5, "PRO-FORMA SALES INVOICE FREE ITEM(S) TOTAL :", 'LB', 0, 'L');
            $pdf->cell(12, 5, $freeProfItemQty, 'B', 0, 'R'); 
            $pdf->cell(87, 5, "", 'B', 0, 'R');
            $pdf->cell(32, 5, "", 'B', 0, 'R');
            $pdf->cell(32, 5, "", 'B', 0, 'R');
            $pdf->cell(32, 5, "", 'BR', 0, 'R');
            ...
            $pdf->Ln();
            $pdf->cell(25, 5, "Pro-forma Sales Invoice Free Item Count : " . $freeProfItemCount, '', 0, 'L');
            $pdf->Ln(10);
        }

        $this->createPiTableHeader($pdf,'Purchase Invoice');
        ...
        if( !empty($flatFreePiItems)){
            $this->createPiTableHeader($pdf, 'Purchase Invoice - Free Item(s)');
            foreach( $flatFreePiItems as $freePi){
 
                $pdf->cell(30, 6, $freePi['piNo'], 'LBR', 0, 'C');
                $pdf->cell(20, 6, $freePi['piDate'], 'BR', 0, 'C', 0);
                $pdf->cell(29, 6, $freePi['vendor_invoice_no'], 'BR', 0, 'C', 0);
                $pdf->cell(30, 6, $freePi['po_no'], 'BR', 0, 'C', 0);
                $pdf->cell(20, 6, $freePi['item'], 'BR', 0, 'C', 0);
                $pdf->cell(90, 6, $freePi['idesc'], 'BR', 0, 'L', 0);
                $pdf->cell(20, 6, $freePi['uom'], 'BR', 0, 'C', 0);
                $pdf->cell(20, 6, $freePi['qty'], 'BR', 0, 'C', 0);
                $pdf->cell(40, 6, 0, 'BR', 0, 'R', 0); //net price
                $pdf->cell(35, 6, 0, 'BR', 0, 'R', 0);  //discounted price
                $pdf->cell(35, 6, 0, 'BR', 0, 'R', 0);  //gross price
                $pdf->cell(32, 6, 0, 'BR', 0, 'R', 0);  //net amount
                $pdf->cell(32, 6, 0, 'BR', 0, 'R', 0);  //discounted amount
                $pdf->cell(32, 6, 0, 'BR', 0, 'R', 0);  //gross amount

                $freePiItemCount    ++;
                $freePiItemQty      += $freePi['qty'];
                $pdf->Ln();
            
            }
            $pdf->setFont('Arial', 'B', 10);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->cell(239, 5, "PURCHASE INVOICE FREE ITEM(S) TOTAL :", 'LB', 0, 'L');
            $pdf->cell(20, 5, $freePiItemQty, 'B', 0, 'R'); 
            $pdf->cell(110, 5, "", 'B', 0, 'R');
            $pdf->cell(32, 5, "", 'B', 0, 'R');
            $pdf->cell(32, 5, "", 'B', 0, 'R');
            $pdf->cell(32, 5, "", 'BR', 0, 'R');
            ...
            $pdf->Ln();
            $pdf->cell(25, 5, "Purchase Invoice Free Item Count : " . $freePiItemCount, '', 0, 'L');
            $pdf->Ln(10);
        }
        ...
        /*   CREDIT MEMO     */
         if( !empty($cmLineMerge) ){
            ...
            foreach($cmLineMerge as $cmd)
            {
                ...
                $pdf->cell(29, 6, $cmd['ref'], 'BR', 0, 'C', 0); //ref#
                $pdf->cell(30, 6, $cmd['po'], 'BR', 0, 'L', 0); //po
                ...
                $pdf->cell(40, 6, number_format($netPricePi, 2), 'BR', 0, 'R', 0); //net price
                $pdf->cell(35, 6, number_format($discountedPricePi,2), 'BR', 0, 'R', 0); //discounted price
                $pdf->cell(35, 6, number_format($cmd['price'],2), 'BR', 0, 'R', 0); //unit price
                $pdf->cell(32, 6, number_format($netPricePi * $cmd['qty'], 2), 'BR', 0, 'R', 0); //net amount
                $pdf->cell(32, 6, number_format($discountedPricePi * $cmd['qty'],2), 'BR', 0, 'R', 0); //discounted amount
                $pdf->cell(32, 6, number_format($cmd['price'] * $cmd['qty'],2), 'BR', 0, 'R', 0); //gross amount
        ...
        $pdf->setFont('Arial', 'B', 10);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->cell(239, 5, "TOTAL PURCHASE INVOICE(PI) (Net of Credit Memo) :", 0, 0, 'L');
        $pdf->cell(20, 5, $purchaseInvoiceItemTotalQty + $cmItemQty, 0, 0, 'R'); 
        $pdf->cell(110, 5, "", 0, 0, 'R');
        $pdf->cell(32, 5, "P " . number_format($purchaseInvoiceItemTotalNetAmt + $cmNetAmount,2), 0, 0, 'R');
        $pdf->cell(32, 5, "P " . number_format($purchaseInvoiceItemTotalDiscAmt + $cmDiscountedAmt,2), 0, 0, 'R');
        $pdf->cell(32, 5, "P " . number_format($purchaseInvoiceItemTotalGrossAmt + $cmGrossAmount, 2), 0, 0, 'R');
        $pdf->Ln(10);
        ...
        if(!empty($sopCharges))
        {
            ...
            $pdf->Cell(80, 7, 'SOP No', 1, 0, 'C', TRUE);
            $pdf->cell(25, 7, 'Date', 1, 0, 'C', TRUE);
            $pdf->cell(125, 7, 'Description', 1, 0, 'C', TRUE);
            $pdf->Cell(30, 7, 'Amount', 1, 0, 'C', TRUE);
            ...
            foreach($sopCharges as $charge)
            {
                ...
                $pdf->Ln();
                if($i == 0){
                    $pdf->cell(80, $height, $charge['sop_no'], 'LBR', 0, 'C');              
                    $pdf->cell(25, $height, date("Y-m-d",strtotime($charge['datetime_created'])), 'BR', 0, 'C');
                    $pdf->cell(125, 5, $charge['description'], 'BR', 0, 'C');
        ...
        if(!empty($getVarianceAdj)){
            $pdf->Ln(2);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Cell(40, 0, 'VARIANCE ADJUSTMENTS :', 0, 0, 'L');
            $pdf->Ln(2);
            $pdf->SetTextColor(201, 201, 201);
            $pdf->SetFillColor(35, 35, 35);  
            $pdf->setFont('Arial', '', 9);
            $pdf->Cell(80, 7, 'Adjustment No', 1, 0, 'C', TRUE);
            $pdf->cell(25, 7, 'Date', 1, 0, 'C', TRUE);
            $pdf->cell(125, 7, 'Description', 1, 0, 'C', TRUE);
            $pdf->Cell(30, 7, 'Amount', 1, 0, 'C', TRUE);

            $pdf->SetTextColor(0, 0, 0);
            foreach($getVarianceAdj as $vAdj)
            {                
                $pdf->Ln();
                $pdf->cell(80, 5, $vAdj['adj_no'], 'LBR', 0, 'C');              
                $pdf->cell(25, 5, date("Y-m-d",strtotime($vAdj['adj_date'])), 'BR', 0, 'C');
                $pdf->cell(125, 5, $vAdj['description'], 'BR', 0, 'C');
                $pdf->cell(30, 5, number_format($vAdj['amount'],2), 'BR', 0, 'R'); 
                $varianceAdjustmentTotal += $vAdj['amount']  ;    
            }

            $pdf->Ln();
            $pdf->setFont('Arial', 'B', 10);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->cell(230, 5, "TOTAL Variance Adjustments :", 'LB', 0, 'L', 0);
            $pdf->cell(30, 5, "P " . number_format($varianceAdjustmentTotal, 2), 'BR', 0, 'R');
            $pdf->Ln(15);
        }
        /* VARIANCE ADJUSTMENTS  */
        ...
        $pdf->Cell(105, 0, 'TOTAL CRF/CV Amount : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, "P ". number_format($proformaItemTotalGrossAmt + $sopTotalCharges + $sopTotalDeductionW12,2), 0, 0, 'R');
        $pdf->Ln(10);
        ...
        $pdf->Cell(105, 0, 'NET PURCHASE INVOICE (PI) Amount : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, "P ". number_format( ($purchaseInvoiceItemTotalGrossAmt + $sopTotalCharges + $cmGrossAmount + $sopTotalDeductionW12) ,2), 0, 0, 'R');
        $pdf->Ln(10);
        ...
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->SetFont('Arial', 'BU', 10);
        $pdf->cell(30, 0, "P ". number_format( ($purchaseInvoiceItemTotalGrossAmt + $sopTotalCharges + $cmGrossAmount + $sopTotalDeductionW12) * -1,2), 0, 0, 'R');
        $pdf->setFont('Arial', 'B', 10);
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Variance (Gross Amount) : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, "P ". number_format( ($proformaItemTotalGrossAmt + $sopTotalCharges + $sopTotalDeductionW12) + (($purchaseInvoiceItemTotalGrossAmt + $sopTotalCharges + $cmGrossAmount + $sopTotalDeductionW12) * -1)  ,2), 0, 0, 'R');
        $pdf->Ln(10);
        ...
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, number_format( $proformaItemCount + $freeProfItemCount,2), 0, 0, 'R');
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Purchase Invoice Item Count : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->SetFont('Arial', 'BU', 10);
        $pdf->cell(30, 0, number_format( ($purchaseInvoiceItemCount + $freePiItemCount - $cmItemCount) * -1,2), 0, 0, 'R');
        $pdf->setFont('Arial', 'B', 10);
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Variance in Item Count : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, number_format( ($proformaItemCount + $freeProfItemCount) - ($purchaseInvoiceItemCount + $freePiItemCount - $cmItemCount) ,2), 0, 0, 'R');
        $pdf->Ln(10);

        $pdf->Cell(105, 0, 'Pro-forma Sales Invoice Item Quantity Total : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, number_format( $proformaItemTotalQty + $freeProfItemQty,2), 0, 0, 'R');
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Purchase Invoice Item Quantity Total  : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->SetFont('Arial', 'BU', 10);
        $pdf->cell(30, 0, number_format( ($purchaseInvoiceItemTotalQty + $freePiItemQty + $cmItemQty) * -1,2), 0, 0, 'R');
        $pdf->setFont('Arial', 'B', 10);
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Variance in Item Quantity : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, number_format( ($proformaItemTotalQty  + $freeProfItemQty) - ($purchaseInvoiceItemTotalQty + $freePiItemQty + $cmItemQty) ,2), 0, 0, 'R');
        $pdf->Ln(10);

        $pdf->Cell(105, 0, 'Variance (Discounted Amount)  : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, "P ". number_format( $proformaItemTotalDiscAmt + ($purchaseInvoiceItemTotalDiscAmt + $cmDiscountedAmt) * -1,2), 0, 0, 'R');
        $varianceDiscountedTotal = round(($proformaItemTotalDiscAmt  + ($purchaseInvoiceItemTotalDiscAmt + $cmDiscountedAmt) * -1),2) ;
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Variance Adjustment  : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, "P ". number_format($varianceAdjustmentTotal,2), 0, 0, 'R');
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Mentions From Other SOP : ', 0, 0, 'L'); 
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->SetFont('Arial', 'BU', 10);
        ...
        $pdf->Ln(5);        
        $pdf->cell(230, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, "P ". number_format( $varianceDiscountedTotal + $varianceAdjustmentTotal + $sopVarianceMentionTotal ,2), 0, 0, 'R');
        ...
        // for variance
        if($fetch_data['saveVar'] == '1' && $varianceDiscountedTotal != 0.00 ){
            $data = $this->proformavspi_model->getData2('variance','crf_id', $fetch_data['crfId']);
            if( is_null($data)){ //if wala pa na save ang variance
                $varianceData = ['crf_id'            => $fetch_data['crfId'],
                                 'supplier_id'       => $fetch_data['supId'],
                                 'variance_amount'   => $varianceDiscountedTotal,
                                 'user_id'           => $this->session->userdata('user_id') ];
                $this->db->insert('variance',$varianceData);
                $varId = $this->db->insert_id();
                $ledger = ['variance_id'  => $varId,
                           'debit'        => $varianceDiscountedTotal,
                           'credit'       => 0,
                           'debit_orig'   => $varianceDiscountedTotal,
                           'adjustment'   => 0 ];
                $this->db->insert('variance_ledger',$ledger);
            }           
        }
    ...
    private function createProfTableHeader($pdf, $title)
    {
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(40, 0, $title, 0, 0, 'L');
        $pdf->Ln(3);
    ...
    private function createPiTableHeader($pdf, $title)
    {
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(40, 0, $title, 0, 0, 'L');
        $pdf->Ln(3);
    ...
    public function searchPoforTubigon()
    {
        $fetch_data = json_decode($this->input->raw_input_stream, true);
        $getPo      = $this->proformavspi_model->searchPo($fetch_data['supId'], $fetch_data['po']); 

        JSONResponse($getPo);
    }

    public function uploadPiTubigon()
    {
        $fetch_data   = $this->input->post(NULL,TRUE);
        $msg          = [];
        $itemNotFound = [];
        ...
        $this->db->trans_start();
        $supCode    = $this->proformavspi_model->getSupplierData($fetch_data['pitub_sup'],'supplier_id')['supplier_code'];
        $checkPo    = $this->proformavspi_model->getPoData($fetch_data['poId'], 'po_header_id', 'po_header_id');

        if($checkPo){

            $getExt          =  pathinfo($_FILES['pitub_File']['name'], PATHINFO_EXTENSION);  
            $getFileName     =  substr(pathinfo($_FILES['pitub_File']['name'], PATHINFO_FILENAME),0,3); 

            if( $getExt == 'txt' || $getExt == 'TXT' ){
                $isAllowed = TRUE ;
                $piContent = file_get_contents($_FILES['pitub_File']['tmp_name']) ;
                $line      = explode("\n", $piContent);
                $totalLine = count($line);
                ...
                {                
                    if ( $line[$n] !== NULL || $line[$n] !== "" ) {
                        ...
                        if ($countRefined == 20 && $n == 1) { //items line
                            $itemsLine ++ ;
                            $checkItem = $this->proformavspi_model->checkItem('itemcode_loc',trim($refined[2]));
                            if(!$checkItem){
                                $itemNotFound[$n] = trim($refined[2]);                            
            ...
            if( $isAllowed && empty($itemNotFound) && $isHeader && $itemsLine > 0 ){
                for ($c = 0; $c < count($_FILES['pitub_File']['name']); $c++) 
                {   
                    $fileName  = $_FILES['pitub_File']['name'];
                    $Temp      = $_FILES['pitub_File']['tmp_name'];
                    $piContent = file_get_contents($_FILES['pitub_File']['tmp_name']) ;
                    $line      = explode("\n", $piContent);
                    $totalLine = count($line);
                    ...
                    for ($l = 0; $l < $totalLine; $l++) 
                    {
                        if ($line[$l] != NULL) {
                            ...
                            if ($countRefined == 7) { // header
                                ...
                                if ($supCode == trim($refined[6])) {//validate if same ang sa selected supplier sa supplier nga  naa sa texfile
                                    $supplierErr        = FALSE ;
                                    $piNo               = trim($refined[0]);
                                    $vendorInvoiceNo    = trim($refined[1]);
                                    $postingDate        = date("Y-m-d", strtotime(trim($refined[2])));
                                    ...
                                    $piId = $this->proformavspi_model->uploadHeader($piNo, $vendorInvoiceNo, $postingDate, $amtIncVat, $checkPo->po_header_id, $fetch_data['pitub_sup'], 13);
                                    ...
                                    if(!$piId){
                                        ...
                                        $saveDocument = [   'document_name'  => $fileName,
                                                            ...
                                                            'document_path'  => "files/PROFORMAVSPI/".$fileName,
                                                            'uploaded_on'    => date("Y-m-d H:i:s"),
                                                            'supplier_id'    => $fetch_data['pitub_sup'],
                                                            'customer_code'  => 13, //ldi tubigon
                                                            'user_id'        => $this->session->userdata('user_id')] ;                                                    
                                        $this->db->insert('uploaded_transaction_documents', $saveDocument);
                                        move_uploaded_file($Temp,"files/PROFORMAVSPI/".$fileName);
                                    }
                            ...
                            if ($countRefined == 20 && $refined[6] != 0 ) { // line and qty != 0
                                ...
                                $lineDiscPercent    = trim($refined[18]) ; //additional
                                $lineDiscAmount     = str_replace(',','', trim($refined[19])) ;
                                $getPiId            = $this->proformavspi_model->getPiHeadData($piNo,$fetch_data['pitub_sup'])['pi_head_id'];                                 
                                $this->proformavspi_model->uploadLine($itemCode, $desc, $uom, $qty, $directUnitCost, $unitCostLcy, $amt, $amtIncVat, $unitCost, $lineAmt, $qtyPerUom, $uomCode, $getPiId,$lineDiscPercent,$lineDiscAmount,$free);
                            }
                        }
                    }
        ...
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            $error = array('action' => 'Uploading Purchase Invoice - Tubigon', 'error_msg' => $this->db->error()); //Log error message to `error_log` table
            $this->db->insert('error_log', $error);
            $msg = ['info'=> 'Error','message' => 'Error Uploading Purchase Invoice - Tubigon!'];
        
        } else if(!$isAllowed){
            $msg = [ 'info' => 'Error', 'message' => 'Invalid file extension detected!' ];
        } else if(!$isHeader){
            $msg = ['info'=> 'Error','message' => 'Text file has no header!'];
        } else if( $itemsLine == 0 ){
            $msg = ['info'=> 'Error','message' => 'Text file has no line!'];
        } else if($piExist){
            $msg = ['info'=> 'Error','message' => 'PI already exists!'];
        } else if($supplierErr){
            $msg = ['info'=> 'Error','message' => 'Selected supplier does not match with the supplier in the textfile!'];
        } else {
            $msg = ['info'=> 'Success','message' => 'Purchase Invoice successfully saved!'];
        }
    ...
    public function piStatistics()
    {
        $statistics = $this->proformavspi_model->getPiStatistics();
