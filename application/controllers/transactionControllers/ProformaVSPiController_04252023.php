...
class ProformaVSPiController extends CI_Controller
{
    ...
    public function printPDFMatching2($transactionId,$profLineMerge, $purchLineMerge, $cmLineMerge, $flatFreeProfItems, $flatFreePiItems, $fetch_data)
    {
        ...
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->SetFont('Arial', 'BU', 10);
        $pdf->cell(30, 0, number_format( ($purchaseInvoiceItemCount + $freePiItemCount) * -1,2), 0, 0, 'R');
        $pdf->setFont('Arial', 'B', 10);
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Variance in Item Count : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, number_format( ($proformaItemCount + $freeProfItemCount) - ($purchaseInvoiceItemCount + $freePiItemCount) ,2), 0, 0, 'R');
        $pdf->Ln(10);
        ...
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->SetFont('Arial', 'BU', 10);
        $pdf->cell(30, 0, number_format( ($purchaseInvoiceItemTotalQty + $freePiItemQty) * -1,2), 0, 0, 'R');
        $pdf->setFont('Arial', 'B', 10);
        $pdf->Ln(5);
        $pdf->Cell(105, 0, 'Variance in Item Quantity : ', 0, 0, 'L');
        $pdf->cell(125, 0, "", 0, 0, 'R');
        $pdf->cell(30, 0, number_format( ($proformaItemTotalQty  + $freeProfItemQty) - ($purchaseInvoiceItemTotalQty + $freePiItemQty) ,2), 0, 0, 'R');
        $pdf->Ln(10);
