window.myApp.controller('proformavscrfhistory-controller', function($scope, $http) {
    ...
    $scope.generateTHistory = function() {
        ...
        $http({
            ...
            method: 'POST',
            url: $base_url + 'generateProfvCrfHistory',
            data: $.param(data),
            responseType: 'json'
        }).then(function successCallback(response) {
            if (response.data != '') {
    ...
    $scope.viewDocument = function(data) {
        window.open($base_url + 'files/Reports/ProformaVsCrf/' + data.filename);
    }

    $scope.deleteDocument = function(data){
        $http({
            method: 'POST',
            url: $base_url + "deleteprofvscrf",
